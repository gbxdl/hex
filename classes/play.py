...
class play:
    ...
    def run(self):
        if self.gameState.guiOn: #GUI on
            print('GUI is on')
            window = Tk()
            GUI = gui(window, self.gameState)
            self.GUI = GUI
            GUI.initInteract()
            if self.gameState.humans == [False,False]:
                self.gamePlay(None,GUI)
            else:
                GUI.window.bind("<Button-1>", lambda event: self.gamePlay(event,GUI), GUI) #lambda takes event so then you can explicitly pass it together with something else.
            window.mainloop()
        else: #GUI off
            return self.gamePlay(None,None) #call gameplay without event and without GUI.
    ...
    def gamePlay(self,event,GUI):        
        ...
        while keepMakingMoves:
            ...
            if move==0:
                return
            self.gameState.position[move]=self.gameState.onMove #change position according to last move.
    ...
    def playMove(self,event,GUI):
        if self.gameState.humans[self.gameState.onMove-1] == True: #if a human is on move
            move = self.gameState.man.makeMove(self.gameState, event, GUI.fromTheEdge, GUI.width)
            if move==0: 
                return [move,False]
        else: #bot is on move
    ...
    def noMorePlay(self,GUI,printWinner):
        ...
        if self.gameState.showFinalPosition:
            GUI.drawPosition()
